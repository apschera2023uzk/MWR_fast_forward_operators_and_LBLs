...
def parse_arguments():
    ...
    outfile = "4campaigns_ARMS2_gb_processed_TBs_from_rs.nc"
...
def check4NANs_in_time_crop_ele(ds, t_index):
    ...
    print("shpae of temp_arr (t should be second: ", np.shape(temp_arr))
...
def create_input4arms_gb_per_elevation(ds, args, elev_index,\
    ...
    for total_index, (i, j) in enumerate(valid_indices):
        profile_indices.append(total_index)
        level_pressures[:, total_index]    = ds["Level_Pressure"].values[:, i]
        level_temperatures[:, total_index] = ds["Level_Temperature"].values[:, i]
        level_wvs[:, total_index]          = ds["Level_H2O"].values[:, i]
        level_ppmvs[:, total_index]        = ds["Level_ppmvs"].values[:, i]
        level_liq[:, total_index]          = ds["Level_Liquid"].values[:, i]
        level_rhs[:, total_index]          = ds["Level_RH"].values[:, i]
        level_z[:, total_index]            = ds["Level_z"].values[:, i]
        srf_pressures[total_index]         = ds["Surface_Pressure"].values[i]
        srf_temperatures[total_index]      = ds["Temperature_2M"].values[i]
        srf_wvs[total_index]               = ds["H2O_2M"].values[i]
        srf_altitude[total_index]          = ds["Surface_Altitude"].values[i]
    ...
    outfilename = outpath+"arms_gb2_inputs.nc"
    write_armsgb_input_nc(
        profile_indices,
        level_pressures, level_temperatures, level_wvs, level_ppmvs,
        level_liq, level_z, level_rhs,
...
def read_outputs_arms_gb(ds, infile, valid_indices):
    ...
    TBs1 = np.full((len(ds["time"].values),\
        len(ds["N_Channels"].values), len(ds["elevation"].values)), np.nan)
...
def get_valid_profiles(ds):
    ...
    for elev_index in range(n_elevs):
        ...
        for i in range(n_times):
            if not check4NANs_in_time_crop_ele(ds, i):
                valid_indices.append((i, elev_index))
...
def derive_TBs4ARMS_gb_per_elevation(ds, args, n_levels=n_levels,\
    ...
    for elev_index in range(len(ds["elevation"].values)):
        ...
        ex_script = str(os.path.expanduser("~/MWR_fast_forward_operators_and_LBLs/python_src/proc/run_arms_gb2.sh"))
    ...
    TBs_ARMS_gb = xr.DataArray(
        all_tbs,
        dims=("time", "N_Channels", "elevation"),
    )
    ds["TBs_ARMS_gb2"] = TBs_ARMS_gb
