...
def parse_arguments():
    ...
    outfile = "4campaigns_RTTOV_gb_processed_TBs_from_rs.nc"    
...
def check4NANs_in_time_crop_ele(ds, t_index, ele_index):
    # Extract the relevant DataArrays for the given indices
    temp_arr = ds["Level_Temperature"].values[:, t_index]
    ppmvs_arr = ds["Level_ppmvs"].values[:, t_index]
    press_arr = ds["Level_Pressure"].values[:, t_index]
    liquid_arr = ds["Level_Liquid"].values[:, t_index]
...
def create_RTTOV_gb_in_profiles(ds, args, batch):
    ...
    for i in batch:    
        for k, elevation in enumerate(ds["elevation"].values):
            if check4NANs_in_time_crop_ele(ds, i,k):
                pass
            else:
                profile1 = write1profile2str(\
                         ds["Level_Temperature"].values[:,i],\
                         ds["Level_ppmvs"].values[:,i],\
                         len(ds["Level_ppmvs"].values[:,i]),\
                         ds["Level_Pressure"].values[:,i],\
                         ds["Level_Liquid"].values[:,i],\
                         height_in_km=ds["Surface_Altitude"].values[i],\
...
def get_rttov_outputs(valid_indices, rttovgb_outfile=\
        "/home/aki/RTTOV-gb/rttov_test/test_example_k.1/output_example_k.dat.gfortran-openmp",\
                   batch_size=batch_size,n_levels=n_levels, n_elev =10):
    print("Reading in RTTOV-gb output from: ", rttovgb_outfile)
    
    tbs = np.full((batch_size, 14,n_elev), np.nan)
    trans = np.full((batch_size, 14,n_elev), np.nan)
    # time, channel, elevation
    trans_by_lev=np.full((batch_size,n_levels, 14, n_elev), np.nan)
    jacs_by_lev=np.full((batch_size, n_levels, 14, n_elev, 4), np.nan)
    ...
    for i, line in enumerate(file.readlines()):
        ...
        if "Profile      " in line:
            ...
            # bie profile Idex = 247 ist valid_indices zu klein...

            rs_time_idx, ele_idx = valid_indices[prof_idx] 
            print("rs_time_idx: ",rs_time_idx, "ele_idx:",ele_idx )
            rs_time_idx = rs_time_idx%batch_size
            print(" rs_time_idx after mod: ",rs_time_idx)
            #####################################
        ...
        if switch and switch_count<2:
            ...
            tb_string = ""
            switch_count = 0
            tbs[rs_time_idx, :, ele_idx] = np.array(tbs_rs)
        ...
        if switch_t and switcht_count<2:
            ...
            trans_string = ""
            switcht_count = 0
            trans[rs_time_idx, :, ele_idx]=np.array(tot_trans_by_chan)
        ...
        if sw_trans_by_lev and sw_c_trans_by_lev<n_levels:
            ...
            for j, line in enumerate(liste):
                ...
                if len(trans_by_lev1)<4:
                    ...
                    trans_by_lev[rs_time_idx, j,10:, ele_idx]=\
        ...
        if sw_jacs_by_lev and sw_c_jacs_by_lev<n_levels+3:
            ...
            for j, line in enumerate(liste):
                werte = line.split()
                jacs_by_lev[rs_time_idx, j, ch_idx, ele_idx, :]=\
                    werte[1:]
            string_jc_by_lev=""
            sw_jacs_by_lev = False
            sw_c_jacs_by_lev= 0
...
def add_rttov_variables_to_ds(
    ...
    ds['TBs_RTTOV_gb'] = ( (time_dim, n_channels_dim, elevation_dim), tbs_data )
    ...
    ds['ttrans_RTTOV_gb'] = ( (time_dim, n_channels_dim, elevation_dim), trans_data )
    ...
    ds['levtrans_RTTOV_gb'] = ( (time_dim, n_levels_dim, n_channels_dim, elevation_dim), trans_by_lev_data )
    ...
    # Jacobian_p_RTTOV_gb'
    ds['Jacobian_p_RTTOV_gb'] = ((time_dim, n_levels_dim),\
       jac_p ) 
    ...
    ds['Jacobian_T_RTTOV_gb'] = ( (time_dim, n_levels_dim, n_channels_dim, elevation_dim), jac_t )
    ...
    ds['Jacobian_ppmv_RTTOV_gb'] = ( (time_dim, n_levels_dim, n_channels_dim, elevation_dim), jac_ppmv )
    ...
    ds['Jacobian_liq_RTTOV_gb'] = ( (time_dim, n_levels_dim, n_channels_dim, elevation_dim), jac_liq )
